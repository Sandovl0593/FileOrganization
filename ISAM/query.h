...
bool select_query(ISAMFile& isam, string table, bool all_at, vector<string> atributes, string k_atr, string k_begin, string k_end, Token::Type index, Token::Type comp) {
    if (table == "player") {
        ...
        if (index == Token::SEQ) {
            ...
            if (table == "player") {
                ...
                if (k_atr == "id") {
                    if (comp == Token::BETWEEN)
                        res = isam.rangeSearchPlayer(stoll(k_begin), stoll(k_end));
                    else if (comp == Token::LESS)
                        res = isam.rangeSearchPlayer(0, stoll(k_begin) - 1);
                    else if (comp == Token::GREATER)
                        res = isam.rangeSearchPlayer(stoll(k_begin) + 1, LONG_LONG_MAX);
                    else if (comp == Token::LESSEQUAL)
                        res = isam.rangeSearchPlayer(0, stoll(k_begin));
                    else if (comp == Token::GREATEREQUAL)
                        res = isam.rangeSearchPlayer(stoll(k_begin), LONG_LONG_MAX);
                } else {
                    // Si necesitas buscar por otros atributos, necesitarías adaptar o mejorar tus funciones ISAM para soportarlo
                    cout << "Búsqueda ISAM por atributo no soportada para 'country' u otros campos aún no configurados." << endl;
                    return false;
                }
            } else if (table == "game") {
                vector<Game> res;
                if (k_atr == "season_id") {
                    if (comp == Token::BETWEEN)
                        res = isam.rangeSearchGame(stoll(k_begin), stoll(k_end));
                    else if (comp == Token::LESS)
                        res = isam.rangeSearchGame(0, stoll(k_begin) - 1);
                    else if (comp == Token::GREATER)
                        res = isam.rangeSearchGame(stoll(k_begin) + 1, LONG_LONG_MAX);
                    else if (comp == Token::LESSEQUAL)
                        res = isam.rangeSearchGame(0, stoll(k_begin));
                    else if (comp == Token::GREATEREQUAL)
                        res = isam.rangeSearchGame(stoll(k_begin), LONG_LONG_MAX);
                } else {
                    // Similar a los jugadores, adaptar para otros campos si es necesario
                    cout << "Búsqueda ISAM por atributo no soportada para 'team_name_home' u otros campos aún no configurados." << endl;
                    return false;
                }
            }

            // Aquí iría el código para procesar los resultados obtenidos y mostrarlos
            // Por ejemplo:
            for (auto& player : res) {
                player.showData();  // Suponiendo que Player tiene un método showData
            }
        }
